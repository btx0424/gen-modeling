...
class EqM(nn.Module):
    ...
    @torch.inference_mode()
    def sample_nag(self, num_samples: int, device: torch.device, num_steps: int, stepsize: float, mu: float) -> torch.Tensor:
        dtype = next(self.parameters()).dtype
        x = torch.randn((num_samples,) + self.network.sample_shape, device=device, dtype=dtype)
...
def main() -> None:
    ...
    model = EqM(ImageEqMBackbone(config.hidden_channels, config.num_blocks)).to(device)
