...
class EBM(nn.Module):
    ...
    def compute_loss(self, x_clean: torch.Tensor, config: Config) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    def sample(self, num_samples: int, device: torch.device, config: Config) -> torch.Tensor:
...
def sample_and_save(model: EBM, config: Config, device: torch.device, out_path: Path, metrics_path: Path | None = None) -> dict[str, float]:
    samples = model.sample(config.num_plot_samples, device, config)
...
def main() -> None:
    ...
    model = EBM(EnergyCNN(config.hidden_channels, config.num_blocks)).to(device)
    ...
    for epoch in range(config.train_epochs):
        ...
        for x, _ in pbar:
            ...
            loss, metrics = model.compute_loss(x, config)
            loss.backward()
            optimizer.step()
